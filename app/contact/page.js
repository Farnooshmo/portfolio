...
export default function Contact() {
  ...
  //   const data ={
  //     name: String(event.name.target.value),
  //     email: String(event.email.target.value),
  //     message: String(event.massage.target.value),
  //   }
  // }
  return (
    <div className="flex flex-col justify-items-center items-center py-20 bg-gradient-to-tr from-zinc-600 to-zinc-950 h-screen text-zinc-700">
      ...
      <form onSubmit={handleSubmit} className="mt-8 ml-8 w-full max-w-lg justify-items-center border-y-4 border-pink-800/50 hover:border-pink-800">
        <div className="flex flex-wrap mb-6 mt-2 ">
          ...
          <input
            // value={name}
            // onChange={(e:FormEvent) => setName(e.preventdefault())}
            minLength={2}
            maxLength={15}
            required
            type="text"
